#ifndef _ROS_azimorph_msg_Status_h
#define _ROS_azimorph_msg_Status_h
...
namespace azimorph_msg
{
  ...
  class Status : public ros::Msg
  {
    public:
      ...
      typedef azimorph_msg::Topic _Topics_type;
      _Topics_type st_Topics;
      _Topics_type * Topics;
      uint32_t Nodes_length;
      typedef azimorph_msg::Node _Nodes_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(Topics_lengthT > Topics_length)
        this->Topics = (azimorph_msg::Topic*)realloc(this->Topics, Topics_lengthT * sizeof(azimorph_msg::Topic));
      Topics_length = Topics_lengthT;
      for( uint32_t i = 0; i < Topics_length; i++){
      offset += this->st_Topics.deserialize(inbuffer + offset);
        memcpy( &(this->Topics[i]), &(this->st_Topics), sizeof(azimorph_msg::Topic));
      }
      ...
      if(Nodes_lengthT > Nodes_length)
        this->Nodes = (azimorph_msg::Node*)realloc(this->Nodes, Nodes_lengthT * sizeof(azimorph_msg::Node));
      Nodes_length = Nodes_lengthT;
      for( uint32_t i = 0; i < Nodes_length; i++){
      offset += this->st_Nodes.deserialize(inbuffer + offset);
        memcpy( &(this->Nodes[i]), &(this->st_Nodes), sizeof(azimorph_msg::Node));
      }
     return offset;
    }

    virtual const char * getType() override { return "azimorph_msg/Status"; };
